...
{   
	...
	void ganar(int x,int y)
	{
		...
		// Quien gana en Oblicuo 1� Posicion De izquierda a derecha
		 ganar1=0;
		 ganar2=0;
			int a=y;
			int b=x;
		 ...
		 // Quien gana en oblicuo? 2� Posicion de derecha a izquierda 
	        ganar1=0;
		    ganar2=0;
			 a=y;
			 b=x;
		      	//buscar posici�n de la esquina
		    	while(b<6 && a>0)
		    	{
		    		a--;
		    		b++;
...
//___________________________ATAQUE M�QUINA EN VERTICAL__________________________
		 ...
		 for(int i=0;i<7;i++) //buscamos en todo el tablero
	     		...
	     		if(boton[j][i].getIcon()!=null)
			    	{
				     	...
				     	if(ganarVert==3 && j!=0) //si en alguna columna hay 3 fichas seguidas de la m�quina
				     	{
		   ...
		   ganarVert=0;
			 	...
			 	if(boton[0][posicion].getIcon()!=null) //si no se pude poner ficha en la columna (columna llena)
		 	     { 	
		 	  	posicion=GenerarAleatorio(posicion); //Genera posici�n aleatoria
		 	     }	
		 		 	int k=7;
		 		 //Ir a la �ltima posici�n de la columna	
		 		 	do
		 		 	 {
		 		 	 	k--;
		 		 	 }
